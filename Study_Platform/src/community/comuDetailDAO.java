...
public class comuDetailDAO {
	...
	public ArrayList<comu> getDetailList(int num) {
		...
		try {
			...
			while(rs.next()) {
				ss.add(new comu(rs.getInt("comu_num"), rs.getString("comu_title"), rs.getString("stu_id"), rs.getString("comu_contents"), rs.getDate("comu_date"), rs.getString("comu_replynum")));
